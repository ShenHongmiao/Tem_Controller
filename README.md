...
## 📚 文档导航
- **[加热测试指南](HEATING_TEST_GUIDE.md)** - 详细的加热测试模式使用说明
- **[项目主文档](#系统架构)** - 完整的系统架构和配置说明
- **[快速开始](#使用说明)** - 硬件连接和系统启动指南
- **[故障排除](#故障排除)** - 常见问题解决方案

## 版本信息
- **当前版本**: v1.6
- **更新内容**: 增加加热测试模式，支持固定输出测试和安全启动保护
- **系统状态**: 稳定运行，支持双温度设定点切换控制和加热测试
...
> **📋 详细测试指南**: [点击查看完整的加热测试使用指南](HEATING_TEST_GUIDE.md)  
> 包含详细的配置步骤、安全注意事项、故障排除和测试记录模板
...
### 功能特性
- **测试模式开关**: 通过`HEATING_TEST_MODE`宏控制，1=开启测试，0=正常PID模式
- **PWM固定输出**: 测试模式下PWM占空比固定为`TEST_PWM_DUTY_CYCLE`设定值
- **继电器固定状态**: 测试模式下继电器固定为`TEST_RELAY_STATE`设定状态
- **安全启动保护**: 上电后强制输出关闭并延时2秒，防止意外加热
- **串口监控**: 测试模式下串口显示固定输出状态，便于调试

### 使用方法
1. **开启测试模式**: 将`HEATING_TEST_MODE`改为1，重新编译烧录
2. **PWM测试**: 设置`CONTROL_MODE_PWM`，调整`TEST_PWM_DUTY_CYCLE`值
3. **继电器测试**: 设置`CONTROL_MODE_RELAY`，调整`TEST_RELAY_STATE`值  
4. **恢复正常**: 将`HEATING_TEST_MODE`改为0，重新编译烧录

### 安全保护机制
- **冷启动保护**: 系统上电后无论何种模式都先关闭输出2秒
- **固定输出**: 测试模式忽略PID计算和温度反馈，避免失控
- **显式配置**: 必须修改宏定义重新编译，防止误操作
- **状态提示**: 串口明确显示当前为测试模式，避免混淆

**⚠️ 安全提醒**: 测试模式下请密切监控温度，避免过热损坏设备！

---
**🔗 更多测试信息**: 查看 **[加热测试完整指南](HEATING_TEST_GUIDE.md)** 了解详细的使用步骤、安全注意事项和故障排除方法。
...
7. **加热测试问题**: 查看 **[加热测试故障排除指南](HEATING_TEST_GUIDE.md#故障排除)**
...
- **v1.6**: 新增加热测试模式，支持固定PWM占空比和继电器状态测试，增加上电安全保护机制
...
### 测试与调试
- [🔥 加热测试指南](HEATING_TEST_GUIDE.md) - 详细的加热测试模式操作说明、安全注意事项和故障排除

### 开发文档
- STM32CubeMX配置文件: `Tem_Controller.ioc`
- 构建配置: `CMakeLists.txt`、`CMakePresets.json`
- 链接脚本: `STM32F103XX_FLASH.ld`

### 硬件参考
- 芯片手册: STM32F103C8T6 Reference Manual
- HAL库文档: STM32F1xx HAL Driver User Manual
- FreeRTOS文档: FreeRTOS Real Time Kernel Reference Manual
